...
| 4 | 為什麼困難 | 撞上組合爆炸、分塊的召回代價、腐爛曲線，與六種人的阻力 |
| 5 | 建立關聯 | 切換「解析前／解析後」，看圖從碎的變成連通的 |
| 6 | 約束傳播 | 在有限資源下做決策，看代價沿著圖傳到別的客戶身上 |
| 7 | 動作與寫回 | 撞上權限、樂觀鎖衝突、逾時、部分成功後的補償回滾 |
| 8 | 總結與卓越 | 兩份資料集並排；卓越的五個判準；你還能做什麼 |

每一關的設計原則是**先讓學員撞牆，再給工具**。所有「對答案」都鎖在作答之後。

另有一個獨立分頁 **「如何做出來」**，記錄這個專案的起點、設計取捨、
過程中抓到的三個 bug、驗證方法，以及已知限制。放它的理由跟整份教材一樣：
只看成品學不到東西。

## Stage 4 為什麼存在

前七關做完只要十幾分鐘，新人很容易得到「這也還好」的結論。Stage 4 專門拆掉那個錯覺，
而且每個數字都是算的：

- **組合爆炸** — 5 萬筆客戶資料是 12.5 億組配對。全靠人看是 3,858 人年；
  全丟給模型是 $5,000,000 而且要跑 1.6 年。成本線性下降，問題平方成長。
- **分塊的代價** — 用這份資料集實際計算：依來源系統分塊省下 79% 計算，但召回率只剩 14%，
  因為跨系統配對正是你要找的東西。用統編分塊召回剩 43%，因為多數紀錄沒有統編。
  同一個欄位在「判斷」時最強，在「分堆」時可能最糟。
- **腐爛** — 沒人維護的規則 18 個月從 94% 掉到 45%；有人維護穩定在 86%，但回不到 94%。
- **人的阻力** — 六張卡片，每張有「為什麼會這樣」與「你能做什麼」。
...
| `src/engine/scale.js` | 組合爆炸、分塊召回統計、腐爛模型 | ✅ 真計算（腐爛是模型，參數公開） |
...
八個關卡會自動套用，不需要改任何 UI 程式碼。一份資料集需要提供：
...
- `graph` — Stage 5 的節點座標與連線
- `scenario` — Stage 6 的資源、需求、可選方案（引擎會自己算最佳解）
- `roles` + `actions` — Stage 7 的權限模型與寫回步驟
- `debrief` — Stage 8 的重點

Stage 4 不需要額外資料 —— 它直接拿 `resolution.records` 去算分塊統計。

想拿真實企業的資料練習，這是入口。建議先做的一件事是**把統一編號補進客戶主檔** ——
理由在 Stage 3、Stage 4 和 Stage 8 有完整說明。
